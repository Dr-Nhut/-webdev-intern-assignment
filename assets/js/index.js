...
const app = {
    ...
    renderProducts : function() {
        ...
        if(cartItems.length) {
            htmlProducts = this.shoes.map( shoes => { // Lặp qua file JSON
                //Check xem đã được thêm chưa
    ...
    handleEvents : function() {
        ...
        document.onclick = function(e) {
            ...
            if(element.classList.contains('add-to-cart')) {
                //Thêm product vào localStorage
                let id = +element.getAttribute('data-id');
                let cartItems = localStorage.getItem('shop-items'); //Danh sách sản phâm trong giỏ
                cartItems ? cartItems =  JSON.parse(cartItems) : cartItems = [];
                let product = _this.shoes.find(function(shoes) { //Sản phẩm đang thêm vào
                    return shoes.id === id;
                });
                product.count = 1;
